...
router.post("/", async (req, res) => {
  ...
  if (req.user.isStudent()) {
    return res
      .status(400)
      .send(
        "Only instructors can publish new courses. If you are already a lecturer, please log in with your lecturer account."
      );
  }
...
router.patch("/:_id", async (req, res) => {
  ...
  try {
    ...
    if (!courseFound) {
      return res
        .status(400)
        .send("Course not found. Unable to update course content.");
    }
    ...
    if (courseFound.instructor.equals(req.user._id)) {
      ...
      return res
        .status(403)
        .send("Only the instructor of this course can edit the course.");
    }
  } catch (e) {
    return res.status(500).send(e);
  }
...
router.delete("/:_id", async (req, res) => {
  ...
  try {
    ...
    if (!courseFound) {
      return res.status(400).send("Course not found. Unable to delete course.");
    }
